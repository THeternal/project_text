...
public class DaikenInvestigateDaoImpl extends DaoSupport<DaikenInvestigate> implements DaikenInvestigateDao {
	...
	public List<DaikenInvestigate> investigateList(Integer investigateStatus, String descStr, Boolean isOver,
			Integer pageNo, Integer pageSize, Boolean rewardPrice) {
	...
	@Override
	public Double selectDayUserMoney(Integer type, List<Integer> userId, String dateStart, String dateEnd) {
		Example example = new Example(DaikenInvestigate.class);
		Criteria criteria = example.createCriteria();
		criteria.andEqualTo(KemeanConstant.DATA_DELETED, false);
		if (type != null) {
		...
		List<DaikenInvestigate> dbExample = mapper.selectByExample(example);
		for (DaikenInvestigate daikenInvestigate : dbExample) {
			userMoney += daikenInvestigate.getMaxPeopleNum() * daikenInvestigate.getRewardPrice();
		}
		return userMoney;
	}

	@Override
	public Integer selectUserNum(Integer type, String dateStart, String dateEnd) {
		return mapper.selectUserNum(type, dateStart, dateEnd);
	}

	@Override
	public Integer selectInvestTypeNum(Integer userId, Integer type, String dateStart, String dateEnd) {
		Example example = new Example(DaikenInvestigate.class);
		Criteria criteria = example.createCriteria();
		criteria.andEqualTo(KemeanConstant.DATA_DELETED, false);
		if (StringUtils.isNoneBlank(dateStart)) {
	...
	@Override
	public List<DaikenInvestigate> adminInvestigateList(String title, List<Integer> type, String dateTimeS,
			String dateTimeE, List<Integer> userId, Integer pageNo, Integer pageSize) {
		Example example = new Example(DaikenInvestigate.class);
		Criteria criteria = example.createCriteria();
		criteria.andEqualTo(KemeanConstant.DATA_DELETED, false);
		if (!type.isEmpty()) {
		...
		if (StringUtils.isNoneBlank(dateTimeS)) {
			criteria.andGreaterThanOrEqualTo("endTime", dateTimeS);
		}
		if (StringUtils.isNoneBlank(dateTimeE)) {
			criteria.andLessThanOrEqualTo("endTime", dateTimeE);
		}
	...
	@Override
	public Integer selectPlatformPublishCount(Integer type, List<Integer> userId, String dateStart, String dateEnd) {
		Example example = new Example(DaikenInvestigate.class);
		Criteria criteria = example.createCriteria();
		criteria.andEqualTo(KemeanConstant.DATA_DELETED, false);
		criteria.andEqualTo("isPlatformPublish", true);
		if (type != null) {
			criteria.andEqualTo("type", type);
		}
		if (StringUtils.isNoneBlank(dateStart)) {
